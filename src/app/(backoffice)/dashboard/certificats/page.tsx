      ...
      <Panel title="Registre des certificats" description="Emission automatique apres validation des resultats.">
        <DataTable
          ...
          rows={certificates.map((certificate) => [
            ...
            `${decimalToNumber(certificate.enrollment.averageScore).toFixed(2)} / 20`,
            <div key="actions" className="flex flex-wrap gap-2">
              <Link href={`/certificats/${certificate.id}`} className="rounded-full bg-[color:var(--brand-green)] px-3 py-2 text-xs font-semibold text-white">
                Imprimer
              </Link>
              <Link
                href={`/api/certificates/verify/${certificate.verificationCode}`}
                className="rounded-full border border-[color:var(--stroke)] px-3 py-2 text-xs font-semibold"
              >
                Verifier
              </Link>
            </div>,
