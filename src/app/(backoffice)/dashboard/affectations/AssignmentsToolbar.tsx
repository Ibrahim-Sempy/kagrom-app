  ...
  return (
    <div className="space-y-5">
      <form onSubmit={handleFilterSubmit} className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <SelectField label="Filtrer par employe" name="employeeId" defaultValue={filters.employeeId} options={employees} />
        <SelectField label="Filtrer par foyer" name="householdId" defaultValue={filters.householdId} options={households} />
        <Field label="Filtrer par date de debut" name="startDate" type="date" defaultValue={filters.startDate} />
      ...
      <div className="flex flex-col gap-4 border-t border-[color:var(--stroke)] pt-4 xl:flex-row xl:items-center xl:justify-between">
        <p className="text-sm text-[color:var(--foreground-muted)]">
          {totalItems} affectation{totalItems > 1 ? "s" : ""} trouvee{totalItems > 1 ? "s" : ""} - affichage {from}-{to}
        </p>
